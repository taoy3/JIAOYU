...
public class DownloadManager extends BaseDownload
{
    ...
    private ICallback mCallback = new ICallback.Stub()
    {
        ...
        public void onFinish(DownloadInfo info) throws RemoteException
        {
            ...
            {
                downloadedData.put(info.videoid, getDownloadInfoBySavePath(info.savePath));
            }
            if(listener != null)
            {
                listener.onFinish();
    ...
    public HashMap<String,DownloadInfo> getDownloadingData()
    {
        if(downloadService != null)
        {
            try
            {
                return (HashMap<String,DownloadInfo>)downloadService.getDownloadingData();
            }catch(RemoteException e)
            {
                Logger.e(TAG, e);
            }
        ...
        if(sdCard_list == null && ( sdCard_list = SDCardManager.getExternalStorageDirectory() ) == null)
        {
        ...
        for(int j = 0; j<sdCard_list.size(); j++)
        {
            ...
            {
                String vid = dirs[i];
                DownloadInfo info = getDownloadInfoBySavePath(
                        sdCard_list.get(j).path+YoukuPlayerConfiguration.getDownloadPath()+vid+"/");
                if(info != null && info.getState() != DownloadInfo.STATE_FINISH && info
                        .getState() != DownloadInfo.STATE_CANCEL)
                {
    ...
    private HashMap<String,DownloadInfo> getNewDownloadedData()
    {
        ...
        for(int j = 0; j<sdCard_list.size(); j++)
        {
            ...
            {
                String vid = dirs[i];
                final DownloadInfo d = getDownloadInfoBySavePath(
                        sdCard_list.get(j).path+YoukuPlayerConfiguration.getDownloadPath()+vid+"/");
    ...
    public DownloadInfo getDownloadInfo(String showId, int show_videoseq)
    {
        ...
        while(iter.hasNext())
        {
            ...
            if(showId.equals(info.showid) && info.show_videoseq == show_videoseq && info
                    .getState() != DownloadInfo.STATE_CANCEL)
            {
                return info;
            }
     ...
     * TODO 创建下载任务(批量)
     */
    public void createDownloads(String[] videoIds, String[] videoNames, OnCreateDownloadListener listener)
    {
        setOnCreateDownloadListener(listener);
        try
        {
    ...
    public abstract class OnCreateDownloadReceiver extends BroadcastReceiver
    {
        ...
        public void onReceive(Context context, Intent intent)
        {
            ...
            {
                boolean value = intent.getBooleanExtra(IDownload.KEY_CREATE_DOWNLOAD_IS_NEED_REFRESH, true);
                Logger.d("Download_OnCreateDownloadListener", "onAllReady():"+value);
                onfinish(value);
