...
public class YoukuPlayerView extends PluginOverlay implements DetailMessage, MediaPlayerObserver
{
    ...
    VideoView cc;
    private FrameLayout mFl_volumn;
    private ImageView mIb_video;
    private ProgressBar mProgressBar;
    private FrameLayout mFl_volumn_brig;
    private ImageView mIb_video_brig;
    private ProgressBar mProgressBar_brig;
    //    private FrameLayout mFl_volumn;
    ...
    private void init(Context context)
    {
        ...
        Log.e("mMaxVolumemMax", mMaxVolume+"");
        // 测试-----------------------------------------------------------------------------------------------------------
    ...
    private void initLayout()
    {
        ...
        mFl_volumn = (FrameLayout)findViewById(R.id.fl_volumn);
        mIb_video = (ImageView)mFl_volumn.findViewById(R.id.ib_video);
        mProgressBar = (ProgressBar)mFl_volumn.findViewById(R.id.progressBar);
        //111111111111111111111111111111111111111111111111111
        mFl_volumn_brig = (FrameLayout)findViewById(R.id.fl_volumn_brig);
        mIb_video_brig = (ImageView)mFl_volumn_brig.findViewById(R.id.ib_video_brig);
        mProgressBar_brig = (ProgressBar)mFl_volumn_brig.findViewById(R.id.progressBar_brig);
    ...
    {
        if(( what == MPPErrorCode.MEDIA_INFO_DATA_SOURCE_ERROR || what == MPPErrorCode.MEDIA_INFO_NETWORK_DISSCONNECTED || what == MPPErrorCode.MEDIA_INFO_NETWORK_CHECK || what == MPPErrorCode.MEDIA_INFO_NETWORK_ERROR || what == MPPErrorCode.MEDIA_INFO_PREPARE_TIMEOUT_ERROR ) && null != mMediaPlayerDelegate.videoInfo && !mMediaPlayerDelegate.videoInfo.IsSendVV)
        {
            if(PlayerUtil.isBaiduQvodSource(mMediaPlayerDelegate.videoInfo.mSource))
            {
                Track.onError(mContext, mMediaPlayerDelegate.videoInfo.getVid(), Profile.GUID,
                        StaticsUtil.PLAY_TYPE_NET, PlayCode.VIDEO_LOADING_FAIL, mMediaPlayerDelegate.videoInfo.mSource,
            ...
            }else if(!StaticsUtil.PLAY_TYPE_LOCAL.equals(mMediaPlayerDelegate.videoInfo.playType))
            {
                Track.onError(mContext, mMediaPlayerDelegate.videoInfo.getVid(), Profile.GUID,
                        mMediaPlayerDelegate.videoInfo.playType, PlayCode.VIDEO_LOADING_FAIL,
        ...
        {
            if(what == MPPErrorCode.MEDIA_INFO_DATA_SOURCE_ERROR || what == MPPErrorCode.MEDIA_INFO_NETWORK_CHECK || what == MPPErrorCode.MEDIA_INFO_NETWORK_DISSCONNECTED || what == MPPErrorCode.MEDIA_INFO_SEEK_ERROR)
            {
                Track.onError(mContext, mMediaPlayerDelegate.videoInfo.getVid(), Profile.GUID,
        ...
        // 使用系统播放器播放的时候
        if(!PlayerUtil
                .useUplayer() && what == 1 && mMediaPlayerDelegate != null && mMediaPlayerDelegate.videoInfo != null)
        {
            Track.onError(mContext, mMediaPlayerDelegate.videoInfo.getVid(), Profile.GUID,
                    mMediaPlayerDelegate.videoInfo.playType, PlayCode.VIDEO_LOADING_FAIL,
                    mMediaPlayerDelegate.videoInfo.mSource, mMediaPlayerDelegate.videoInfo.getCurrentQuality(),
    ...
    public void OnCurrentPositionChangeListener(int currentPosition)
    {
        if(mMediaPlayerDelegate != null && mMediaPlayerDelegate.videoInfo != null)
        {
            ...
            // 付费视频需要在10分钟时候发送统计
            if(mMediaPlayerDelegate.videoInfo.paid && !mMediaPlayerDelegate.videoInfo.paidSended && currentPosition/1000 == 600)
            {
                new DisposableHttpCookieTask(PLAY_LOG_URL+"?vid="+mMediaPlayerDelegate.videoInfo.getVid()).start();
                mMediaPlayerDelegate.videoInfo.paidSended = true;
            }
            if(mMediaPlayerDelegate.videoInfo.getLookTen() == 1 && currentPosition/1000>=600)
            {
    ...
    public void resizeVideoView(int percent, boolean force)
    {
        ...
        int orientation = mMediaPlayerDelegate.mediaPlayer == null ? 0 : mMediaPlayerDelegate.mediaPlayer
                .getVideoOrientation();
        if(lastpercent == percent && fullWidth == lastFullWidth && fullHeight == lastFullHeight && lastOrientation == orientation && !force)
        {
            return;
        }
        ...
        RelativeLayout.LayoutParams leftPara = (android.widget.RelativeLayout.LayoutParams)leftSpace.getLayoutParams();
        ...
        RelativeLayout.LayoutParams topPara = (android.widget.RelativeLayout.LayoutParams)topSpace.getLayoutParams();
        ...
        ++showWidth;
        ++showHeight;
        Logger.e("PlayFlow", "changeVideoSize-->"+"showWidth-->"+showWidth+"showHeight-->"+showHeight);
        if(mMediaPlayerDelegate.mediaPlayer != null)
        {
            mMediaPlayerDelegate.mediaPlayer.changeVideoSize(showWidth, showHeight);
        }
     ...
     * @see {@link Plantform}
     */
    public void initialize(YoukuBasePlayerManager mYoukuBaseActivity, int platformId, String pid, String verName, String userAgent, boolean useSystemPlayer)
    {

        initialize(mYoukuBaseActivity, platformId, pid, verName, userAgent, useSystemPlayer, null, null);
    }
    ...
    {
        PackageManager pm = mYoukuBaseActivity.getBaseActivity().getPackageManager();
        String ver = "4.1";
        try
        {
            ver = pm.getPackageInfo(mYoukuBaseActivity.getBaseActivity().getPackageName(), 0).versionName;
        ...
        boolean isTablet = ( this.getResources().getConfiguration().screenLayout&Configuration.SCREENLAYOUT_SIZE_MASK
        )>=Configuration.SCREENLAYOUT_SIZE_LARGE;
        ...
        initialize(mYoukuBaseActivity, 10001, "4e308edfc33936d7", ver, ua, false, -7L,
     ...
     * @see {@link Plantform}
     */
    public void initialize(YoukuBasePlayerManager basePlayerManager, int platformId, String pid, String verName, String userAgent, boolean useSystemPlayer, Long timeStamp, String secret)
    {
        ...
        //        mFl_volumn = mBasePlayerManager.fl_volumn;
    }
    ...
    private void trackPlayerLoad(long duration)
    {
        ...
        AnalyticsWrapper
                .trackExtendCustomEvent(mContext, PlayerStatistics.PALYER_LOAD, PlayerStatistics.PAGE_NAME, null,
    ...
    private Handler mDismissHandler = new Handler()
    {
        ...
        public void handleMessage(Message msg)
        {
            if(msg.what == 0)
            {
                if(mFl_volumn.getVisibility() == View.VISIBLE)
                {
                    mFl_volumn.setVisibility(View.GONE);
                }
                mFl_volumn.setVisibility(View.GONE);
            }else
            {
                if(mFl_volumn_brig.getVisibility() == View.VISIBLE)
                {
                    mFl_volumn_brig.setVisibility(View.GONE);
                }
    ...
    private void onVolumeSlide(float percent)
    {
        if(mVolume == -1)
        {
            ...
            mFl_volumn.setVisibility(View.VISIBLE);
            // 显示
            mIb_video.setBackgroundResource(R.drawable.video_volumn_bg);
        }
    ...
    private void onBrightnessSlide(float percent)
    {
        if(mBrightness<0)
        {
            ...
            mFl_volumn_brig.setVisibility(View.VISIBLE);
        }
        ...
        mProgressBar_brig.setMax((int)( 1.0f*100 ));
        mProgressBar_brig.setProgress((int)( lpa.screenBrightness*100 ));
