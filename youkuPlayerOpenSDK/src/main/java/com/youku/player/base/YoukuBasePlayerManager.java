...
public abstract class YoukuBasePlayerManager extends IBasePlayerManager implements DetailMessage, OrientationChangeCallback
{
    ...
    public void onCreate()
    {
        ...
        if(DEVELOPER_MODE)
        {
            StrictMode.setThreadPolicy(new StrictMode.ThreadPolicy.Builder().detectDiskReads()
                    .detectDiskWrites().detectNetwork() // 这里可以替换为detectAll()
                            // 就包括了磁盘读写和网络I/O
                    .penaltyLog() // 打印logcat，当然也可以定位到dropbox，通过文件保存相应的log
                    .build());
            StrictMode.setVmPolicy(new StrictMode.VmPolicy.Builder().detectLeakedSqlLiteObjects() // 探测SQLite数据库操�?
                    .penaltyLog() // 打印logcat
                    .penaltyDeath().build());
        }
        ...
        try
        {
            YoukuBasePlayerManager.versionName = getBaseActivity().getPackageManager()
                    .getPackageInfo(getBaseActivity().getPackageName(), PackageManager.GET_META_DATA).versionName;
        ...
        if(TextUtils.isEmpty(com.baseproject.utils.Profile.User_Agent))
        {
            ...
            com.baseproject.utils.Profile.initProfile("player",
                    plant+versionName+";Android;"+android.os.Build.VERSION.RELEASE+";"+android.os.Build.MODEL,
                    getBaseActivity());
        }
        ...
        {                                //-------------------->
            ...
            com.youku.player.goplay.Profile
                    .setVideoType_and_PlayerType(com.youku.player.goplay.Profile.FORMAT_FLV_HD, getBaseActivity());
        }else
        {
            YoukuBasePlayerManager.isHighEnd = false;
            com.youku.player.goplay.Profile
                    .setVideoType_and_PlayerType(com.youku.player.goplay.Profile.FORMAT_3GPHD, getBaseActivity());
        }
    ...
    public void initLayoutView(YoukuPlayerView mYoukuPlayerView)
    {                                //在YoukuPlayerView初始化时调用
    ...
    protected void addPlugins()
    {
        getBaseActivity().runOnUiThread(new Runnable()
        {
            ...
            public void run()
            {
                player_holder = (FrameLayout)mYoukuPlayerView.findViewById(R.id.player_holder_all);
    ...
    private void initPlayAndSurface()
    {
        ...
        surfaceHolder
                .addCallback(mediaPlayerDelegate.mediaPlayer);                //此处同时把surface holder句柄赋值给了mediaplayer
        if(!isHighEnd)
        {
    ...
    {
        if(mediaPlayerDelegate != null && mediaPlayerDelegate.mediaPlayer != null && mediaPlayerDelegate.mediaPlayer
                .isListenerInit())
        {
            return;
        }
    ...
    {                                            //---------->内部使用
        ...
        mediaPlayerDelegate.mediaPlayer.setOnErrorListener(new OnErrorListener()
        {
            ...
            public boolean onError(MediaPlayer mp, int what, int extra)
            {
                ...
                if(!isSendPlayBreakEvent && MediaPlayerConfiguration.getInstance()
                        .trackPlayError() && mYoukuPlayerView.realVideoStart && mediaPlayerDelegate != null && mediaPlayerDelegate.videoInfo != null)
                {
            ...
            {
                return what == MPPErrorCode.MEDIA_INFO_PREPARED_AD_CHECK || ( what == MPPErrorCode.MEDIA_INFO_DATA_SOURCE_ERROR && !mediaPlayerDelegate
                        .isAdvShowFinished()
                ) || ( what == MPPErrorCode.MEDIA_INFO_NETWORK_ERROR && mediaPlayerDelegate.isADShowing ) || ( what == MPPErrorCode.MEDIA_INFO_NETWORK_CHECK && mediaPlayerDelegate.isADShowing );
            }
            ...
            private void disposeAdErrorLoss(int what)
            {
                ...
                if(what == MPPErrorCode.MEDIA_INFO_DATA_SOURCE_ERROR && !mediaPlayerDelegate.videoInfo.isAdvEmpty())
                {
                    DisposableStatsUtils
                            .disposeAdLoss(YoukuBasePlayerManager.this.getBaseActivity(), URLContainer.AD_LOSS_STEP4,
                                    SessionUnitil.playEvent_session, URLContainer.AD_LOSS_MF);
                }
                if(what == MPPErrorCode.MEDIA_INFO_PREPARED_AD_CHECK)
                {
                    DisposableStatsUtils
                            .disposeAdLoss(YoukuBasePlayerManager.this.getBaseActivity(), URLContainer.AD_LOSS_STEP6,
                                    SessionUnitil.playEvent_session, URLContainer.AD_LOSS_MF);
                }
            }
        });
        ...
        if(PlayerUtil.useUplayer())
        {
            mediaPlayerDelegate.mediaPlayer.setOnADPlayListener(new OnADPlayListener()
            {
                ...
                public boolean onStartPlayAD(int index)
                {
                    ...
                    if(mediaPlayerDelegate != null && mediaPlayerDelegate.videoInfo != null)
                    {
                        AnalyticsWrapper.adPlayStart(getBaseActivity(), mediaPlayerDelegate.videoInfo);
                    }
                    try
                    {
                    ...
                    if(mediaPlayerDelegate.videoInfo.getCurrentAdvInfo() != null && ( mediaPlayerDelegate.videoInfo
                            .getCurrentAdvInfo().VSC == null || mediaPlayerDelegate.videoInfo.getCurrentAdvInfo().VSC
                            .equalsIgnoreCase("")
                    ))
                    {
                ...
                public boolean onEndPlayAD(int index)
                {
                    ...
                    if(mediaPlayerDelegate != null && mediaPlayerDelegate.videoInfo != null)
                    {
                        AnalyticsWrapper.adPlayEnd(getBaseActivity(), mediaPlayerDelegate.videoInfo);
                    }
        ...
        mediaPlayerDelegate.mediaPlayer.setOnLoadingStatusListener(new OnLoadingStatusListener()
        {
            ...
            public void onEndLoading()
            {
                ...
                if(!firstLoaded && !isFromLocal() && !PreferenceUtil
                        .getPreferenceBoolean(YoukuBasePlayerManager.this.getBaseActivity(), "video_lock", false))
                {
                    if(mediaPlayerDelegate != null && !mediaPlayerDelegate.isFullScreen && mediaPlayerDelegate.videoInfo != null && StaticsUtil.PLAY_TYPE_LOCAL
                            .equals(mediaPlayerDelegate.videoInfo.getPlayType()) || !PlayerUtil
        ...
        mediaPlayerDelegate.mediaPlayer.setOnPlayHeartListener(new OnPlayHeartListener()
        {
            ...
            public void onPlayHeart()
            {
                if(mediaPlayerDelegate != null && mediaPlayerDelegate.videoInfo != null)
                {
                    Track.trackPlayHeart(getBaseActivity(), mediaPlayerDelegate.videoInfo,
        ...
        mediaPlayerDelegate.mediaPlayer.setOnVideoIndexUpdateListener(new OnVideoIndexUpdateListener()
        {
            ...
            public void onVideoIndexUpdate(int currentIndex, int ip)
            {
                ...
                if(mediaPlayerDelegate != null && mediaPlayerDelegate.videoInfo != null)
                {
                    Track.onVideoIndexUpdate(getBaseActivity(), currentIndex, ip,
    ...
    protected void sentonVVBegin()
    {
        ...
        {
            mediaPlayerDelegate.isStartPlay = true;
            if(null != mediaPlayerDelegate.videoInfo && !mediaPlayerDelegate.videoInfo.isFirstLoaded)
            {
                id = mediaPlayerDelegate.videoInfo.getVid();
                mediaPlayerDelegate.videoInfo.isFirstLoaded = true;
    ...
    {                        //-------------内部使用
        ...
        if(StaticsUtil.PLAY_TYPE_LOCAL.equals(mediaPlayerDelegate.videoInfo.getPlayType()))
        {
            ...
            mediaPlayerDelegate.videoInfo.setDurationMills(mediaPlayerDelegate.mediaPlayer.getDuration());
            Logger.e("PlayFow", "本地视频读取时间成功 :"+mediaPlayerDelegate.mediaPlayer.getDuration());
            getBaseActivity().runOnUiThread(new Runnable()
            {
    ...
    public void onPause()
    {
        ...
        if(surfaceHolder != null && mediaPlayerDelegate.mediaPlayer != null && !mediaPlayerDelegate.hasRight)
        {
                ...
                public void run()
                {
                    ...
                    if(null != mediaPlayerDelegate && null != surfaceHolder)
                    {
                        if(!mediaPlayerDelegate.isAdvShowFinished())
                        {
                            ...
                            }else if(mediaPlayerDelegate.mAdType == AdType.AD_TYPE_IMAGE && !isImageADShowing)
                            {
                                updatePlugin(PLUGIN_SHOW_NOT_SET);
                            }
                        ...
                        }else
                        {
                            ...
                            if(mPluginADPlay != null && mPluginADPlay.getVisibility() == View.VISIBLE && mPluginADPlay
                                    .isCountUpdateVisible())
                            {
                                mPluginADPlay.showPlayIcon();
                            }
                        }
    ...
    {                    //------------------->内部使用
        ...
        if(pauseBeforeLoaded && !isImageADShowing && !( mAdDialogHint != null && mAdDialogHint
                .isShowing() && !mWaitingLoginResult
        ))
        {
    ...
    public void onConfigurationChanged(Configuration newConfig)
    {
        ...
        if(Profile.PHONE_BROWSER == Profile.from || mediaPlayerDelegate.videoInfo != null && ( StaticsUtil.PLAY_TYPE_LOCAL
                .equals(mediaPlayerDelegate.videoInfo.getPlayType()) || mediaPlayerDelegate.videoInfo.isHLS
        ))
        {
            return;
        }
        if(mediaPlayerDelegate.isFullScreen && PlayerUtil.isYoukuTablet(YoukuBasePlayerManager.this.getBaseActivity()))
        {
            return;
        }
        // 横屏有两种情�?�?
        if(isLand())
        {
            ...
            }else
            {
                ...
                updatePlugin(!mediaPlayerDelegate.isAdvShowFinished() ? PLUGIN_SHOW_AD_PLAY : PLUGIN_SHOW_NOT_SET);
            ...
            switch(msg.what)
            {
                ...
                case GET_PORT_PARAMS:
                {
                    ...
                    }else
                    {
                        ...
                        if(mediaPlayerDelegate != null && mediaPlayerDelegate.videoInfo != null && StaticsUtil.PLAY_TYPE_LOCAL
    ...
    public void setPlayerFullScreen(final boolean lockSensor)
    {
        ...
        if(lockSensor)
        {
            if(UIUtils.hasGingerbread() && !PreferenceUtil
                    .getPreferenceBoolean(getBaseActivity(), "video_lock", false))
            {
    ...
    public void goFullScreen()
    {
        ...
        }else
        {
            ...
            if(Profile.from != Profile.PHONE_BROWSER && !PlayerUtil.isYoukuTablet(
                    getBaseActivity()) && mediaPlayerDelegate != null && ( mediaPlayerDelegate.videoInfo == null || !StaticsUtil.PLAY_TYPE_LOCAL
                    .equals(mediaPlayerDelegate.videoInfo.getPlayType())
            ))
            {
        ...
        try
        {
            switch(keyCode)
            {
                ...
                case KeyEvent.KEYCODE_BACK:
                    ...
                    {
                        if(mediaPlayerDelegate.isFullScreen && !isFromLocal() && ( mediaPlayerDelegate.videoInfo != null && !mediaPlayerDelegate.videoInfo.isHLS ))
                        {
                            goSmall();
                            return true;
                        }else
    ...
    protected void onkeyback()
    {
        ...
        try
        {
            ...
            if(( mediaPlayerDelegate != null && mediaPlayerDelegate.videoInfo != null ) ? mediaPlayerDelegate.videoInfo.isHLS : false)
            {
                IRVideoWrapper.videoEnd(getBaseActivity());
                return;
            }

            if(!mediaPlayerDelegate.isStartPlay && !mediaPlayerDelegate.isVVBegin998Send)
            {
                if(mediaPlayerDelegate.videoInfo == null || TextUtils.isEmpty(mediaPlayerDelegate.videoInfo.getVid()))
                {
                    Track.onError(getBaseActivity(), id, Device.guid, StaticsUtil.PLAY_TYPE_NET, PlayCode.USER_RETURN,
                            mediaPlayerDelegate.videoInfo == null ? Source.YOUKU : mediaPlayerDelegate.videoInfo.mSource,
                            Profile.videoQuality, 0, mediaPlayerDelegate.isFullScreen);
                    mediaPlayerDelegate.isVVBegin998Send = true;
                }else if(!mediaPlayerDelegate.videoInfo.IsSendVV && !mediaPlayerDelegate.videoInfo.isSendVVEnd)
                {
                    if(mediaPlayerDelegate.isADShowing)
                    {
                        Track.onError(getBaseActivity(), mediaPlayerDelegate.videoInfo.getVid(), Device.guid,
                                mediaPlayerDelegate.videoInfo.playType, PlayCode.VIDEO_ADV_RETURN,
                                mediaPlayerDelegate.videoInfo.mSource,
                                mediaPlayerDelegate.videoInfo.getCurrentQuality(),
                                mediaPlayerDelegate.videoInfo.getProgress(), mediaPlayerDelegate.isFullScreen);
                    }else
                    {
                        Track.onError(getBaseActivity(), mediaPlayerDelegate.videoInfo.getVid(), Device.guid,
                                PlayerUtil.isBaiduQvodSource(
                                        mediaPlayerDelegate.videoInfo.mSource) ? StaticsUtil.PLAY_TYPE_NET : mediaPlayerDelegate.videoInfo.playType,
                                PlayCode.USER_LOADING_RETURN, mediaPlayerDelegate.videoInfo.mSource,
                                mediaPlayerDelegate.videoInfo.getCurrentQuality(),
                                mediaPlayerDelegate.videoInfo.getProgress(), mediaPlayerDelegate.isFullScreen);
                        if(!mediaPlayerDelegate.videoInfo.isAdvEmpty())
                        {
                            DisposableStatsUtils.disposeAdLoss(YoukuBasePlayerManager.this.getBaseActivity(),
                                    URLContainer.AD_LOSS_STEP3, SessionUnitil.playEvent_session,
    ...
    private boolean volumeUp()
    {
        if(!isMute)
        {
            am.setStreamVolume(AudioManager.STREAM_MUSIC, Math.min(am.getStreamVolume(AudioManager.STREAM_MUSIC)+1,
        ...
        }else
        {
            if(currentSound>=0)
            {
                am.setStreamVolume(AudioManager.STREAM_MUSIC,
                        Math.min(currentSound+1, am.getStreamMaxVolume(AudioManager.STREAM_MUSIC)), 0);
            }
    ...
    private boolean volumeDown()
    {
        ...
        }else
        {
            if(currentSound>=0)
            {
                ...
                am.setStreamVolume(AudioManager.STREAM_MUSIC, destSound>=0 ? destSound : 0, 0);
            }
        }
    ...
    private boolean isFromLocal()
    {
        ...
        if(Profile.from == Profile.PHONE_BROWSER || ( mediaPlayerDelegate.videoInfo != null && StaticsUtil.PLAY_TYPE_LOCAL
                .equals(mediaPlayerDelegate.videoInfo.getPlayType())
        ))
    ...
    public void onCreateInitialize()
    {
        ...
        initPlayerPart();
        if(!Util.hasInternet() && ( mediaPlayerDelegate.videoInfo == null || !mediaPlayerDelegate.videoInfo.isCached()
        ))
        {
            // Util.showTips("!Util.hasInternet(this)");
        }else
        {
            if(( mediaPlayerDelegate.videoInfo == null || !mediaPlayerDelegate.videoInfo.isCached() ) && Util
                    .hasInternet() && !Util.isWifi())
            {
                // 提示用户没有wifi
            ...
            // 读取是否自动播放
            getIntentData();
            autoPlay = PreferenceManager.getDefaultSharedPreferences(YoukuBasePlayerManager.this.getBaseActivity())
    ...
    {
        if(name.startsWith("GT-I9228") || name.startsWith("Note") || name.startsWith("9220") || name
                .startsWith("I889") || name.startsWith("I717") || name.startsWith("I9228"))
        {
    ...
    {
        return Profile.USE_SYSTEM_PLAYER || ( mediaPlayerDelegate != null && mediaPlayerDelegate.videoInfo != null && StaticsUtil.PLAY_TYPE_LOCAL
    ...
    private void showPasswordInputDialog(int title)
    {
        ...
        AlertDialog dlg = new AlertDialog.Builder(YoukuPlayerConfiguration.context).setTitle(title)
    ...
    {
        if(mediaPlayerDelegate != null && IMediaPlayerDelegate.mIPayCallBack != null && mediaPlayerDelegate.videoInfo != null)
        {
            mediaPlayerDelegate.release();
            mediaPlayerDelegate.onVVEnd();
            IMediaPlayerDelegate.mIPayCallBack
                    .needPay(mediaPlayerDelegate.videoInfo.getVid(), mediaPlayerDelegate.videoInfo.mPayInfo);
        }
    }
    ...
    public void hideSystemUI(PluginOverlay plugin)
    {
        if(mediaPlayerDelegate.isFullScreen)
        {
            plugin.setSystemUiVisibility(View.SYSTEM_UI_FLAG_HIDE_NAVIGATION|View.SYSTEM_UI_FLAG_FULLSCREEN);
        }
    }
