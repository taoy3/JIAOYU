...
public class LoopImageHolder implements OnPageChangeListener {
    ...
    public View initView(){
        ...
        RelativeLayout loopView = new RelativeLayout(mContext);
        if(params == null) {
            // 设置轮播图的宽和高
            params = new AbsListView.LayoutParams(LayoutParams.MATCH_PARENT, BaseApplication.getScreenHeight()
                    /3);
//            params = new RelativeLayout.LayoutParams(LayoutParams.MATCH_PARENT, UIUtils.getDimens(R.dimen.list_header_hight));
        }
    ...
    private class AutoPlayTask implements Runnable {
        ...
        public void run(){
            if(has_auto_play) {
                ...
                mHandler.postDelayed(this, AUTO_PLAY_TIME);
            }
    ...
    private class ViewPagerAdapter extends PagerAdapter {
        ...
        public Object instantiateItem(ViewGroup container, final int position){
            ...
            if(size>0) {
                Glide.with(mContext).load(getData().get(position%size)).into(imageView);
                //                Picasso.with(mContext).load(getData().get(position%size)).into(imageView);
