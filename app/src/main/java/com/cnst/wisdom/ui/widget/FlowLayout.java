...
public class FlowLayout extends ViewGroup {
	...
	private OnItemClickFListener ls;
	...
	protected void onLayout(boolean changed, int l, int t, int r, int b) {
		if (!mNeedLayout || changed) {// 没有发生改变就不重新布局
			...
			final int linesCount = mLines.size();
			for (int i = 0; i < linesCount; i++) {//一行一行的布局
				final Line oneLine = mLines.get(i);
				oneLine.layoutView(left, top,i);// 布局每一行
	...
	public class Line {
		...
		public void layoutView(int l, int t, int li) {// 行中的内部v布局
			int left = l;
			int top = t;
			...
			if (surplusWidth >= 0) {// 剩余空间
				...
				for (int i = 0; i < count; i++) {
					...
					setViewClickListener(view,this,li,i);
				}
			} else {
				if (count == 1) {
		...
		private void setViewClickListener(final View view, final Line line, final int li, final int i) {
			view.setOnClickListener(new OnClickListener() {
				
				@Override
				public void onClick(View v) {
					if (ls!=null) {
						ls.onItemFClicked(line, view, li*getViewCount()+i, i, li, mLines.size());
					}
				}
			});
		}
	...
	public interface OnItemClickFListener{
		/**
		 * 
		 * @param parent  v所在的行
		 * @param v				被点击的v
		 * @param where	v在布局中排第几个
		 * @param whereInLine	v在所在行中排第几
		 * @param Vcounts		布局中v的数量
		 * @param lineCounts		布局中行的数量
		 */
		public void onItemFClicked(Line parent, View v, int where, int whereInLine, int Vcounts, int lineCounts);
	}
	
	public void setOnItemClickFListener(OnItemClickFListener ls){
		this.ls = ls;
	}
}
